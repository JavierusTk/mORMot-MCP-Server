/// mORMot2 MCP Server - Free Pascal Console Application
...
begin
  ...
  begin
    ...
    else if (Length(S) > 0) and not (S[1] in ['-', '/']) then
      // Bare number = port
  ...
  UseNewTransport := TransportType = mttStdio;
end;
...
begin
  ...
  Server := TMCPHttpServer.Create(Settings);
  try
    Server.ManagerRegistry := Registry;
    Server.Start;
...
begin
  ...
  try
    ...
    if UseNewTransport or (TransportType = mttStdio) then
      RunWithTransport
    else
      RunLegacy; // Use existing TMCPHttpServer for HTTP (tested and stable)
  finally
